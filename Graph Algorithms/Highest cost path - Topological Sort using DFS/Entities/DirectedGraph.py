...
class DirectedGraph:
    ...
    def topoSortDfs(self, vertex, processed_vertices, sorted_vertices, vertices_stack):

        # the vertices stack is a set with all the vertices that are not fully processed yet
        if vertex in vertices_stack:
            return False
        vertices_stack.add(vertex)
        for in_neighbour_vertex in self._dictIn[vertex]:
            if in_neighbour_vertex not in processed_vertices:
                ok = self.topoSortDfs(in_neighbour_vertex, processed_vertices, sorted_vertices, vertices_stack)
                if not ok:
                    return False

        # it means that vertex was fully processed, it doesn't have any inbound neighbours
        processed_vertices.add(vertex)
        sorted_vertices.append(vertex)
        vertices_stack.remove(vertex)
        return True
    ...
    def topological_sort(self):
        ...
        for vertex in self._dictIn.keys():
            if vertex not in visited_vertices:
                ok = self.topoSortDfs(vertex, visited_vertices, sorted_vertices, vertices_stack)
                if not ok:
                    return None
        return sorted_vertices

    def get_highest_cost_path(self, source_vertex, destination_vertex):
        topological_sorted_vertices = self.topological_sort()
        if topological_sorted_vertices is None:
            return None, []

        print("Topological order: ", topological_sorted_vertices)
        predecessors ={}
        distance = {}
        for vertex in self._dictIn:
            distance[vertex] = -math.inf
            predecessors[vertex] = -math.inf
        distance[source_vertex] = 0

        for vertex in topological_sorted_vertices:
            if vertex == destination_vertex:
                break
            for out_vertex in self._dictOut[vertex]:
                if distance[out_vertex] < distance[vertex] + self._dictCosts[(vertex, out_vertex)]:
                    distance[out_vertex] = distance[vertex] + self._dictCosts[(vertex, out_vertex)]
                    predecessors[out_vertex] = vertex

        # this means that there is no path between source_vertex and destination_vertex
        if distance[destination_vertex] == -math.inf:
            return 0, []
        ...
        reversed_path.append(source_vertex)
        return distance[destination_vertex], reversed_path

    def get_number_of_distinct_paths(self, source_vertex, destination_vertex):
        topological_sorted_vertices = self.topological_sort()
        if topological_sorted_vertices is None:
            return None
        ...
        # we consider the path from the destination_vertex to itself so we count that path
        # we consider this only for the destination_vertex
        number_of_paths[destination_vertex] = 1

        index = len(topological_sorted_vertices) - 1
        while index >= 0:
            for in_vertex in self._dictIn[topological_sorted_vertices[index]]:
                number_of_paths[in_vertex] += number_of_paths[topological_sorted_vertices[index]]
            index -= 1
        return number_of_paths[source_vertex]
    ...
    def DFS(self, p_vertex, visited, final_list):

        final_list.append(p_vertex)
        visited.append(p_vertex)
        for vertex in self._dictOut[p_vertex]:
            if vertex not in visited:
                self.DFS(vertex, visited, final_list)

    def TrajanUntil(self, vertex, disc, low, stack):
        print("vertex: ", vertex)
        print("disc: ", disc)
        print("low: ", low)
        print("stack: ", stack)
        disc[vertex] = low[vertex] = vertex
        stack.append(vertex)
        for v in self._dictOut[vertex]:
            if disc[v] == -1:
                self.TrajanUntil(v,disc, low, stack)
    ...
    def Trajan(self):
        ...
        for i in self._dictOut.keys():
            if disc[i] == -1:
                self.TrajanUntil(i,disc,low,stack)



g = DirectedGraph(5)
g.initialize_dictionaries_of_the_graph(5)
g.add_edge(0,1,1)
g.add_edge(1,2,1)
g.add_edge(1,3,4)
g.add_edge(2,4,1)
